...
namespace utfconvert
{
	...
	// 从UTF16 LE编码的字符串创建
	static inline std::string utf16le_to_utf8(const std::u16string& u16str)
	{
	...
	// 从UTF16BE编码字符串创建
	static inline std::string utf16be_to_utf8(const std::u16string& u16str)
	{
		if (u16str.empty()) { return std::string(); }
		const char16_t* p = u16str.data();
		std::u16string::size_type len = u16str.length();
	...
	static inline std::string utf16_to_utf8(const std::u16string& u16str)
	{
		if (u16str.empty()) { return std::string(); }
		//Byte Order Mark
	...
	static inline std::u16string utf8_to_utf16le(const std::string& u8str, bool addbom = false, bool* ok = NULL)
	{
	...
	// 获取转换为UTF-16 BE的字符串
	static inline std::u16string utf8_to_utf16be(const std::string& u8str, bool addbom = false, bool* ok = NULL)
	{
		// 先获取utf16le编码字符串
	...
	static inline std::string utf8_to_utf8bom(const std::string& u8str)
	{
		...
		u8bom.push_back(static_cast<unsigned char>(0xBB));
		u8bom.push_back(static_cast<unsigned char>(0xBF));
		
		std::string::size_type len = u8str.length();
		const char* p = (const char*)(u8str.data());
		// 判断是否具有BOM(判断长度小于3字节的情况)
		if (len > 3 && p[0] == 0xEF && p[1] == 0xBB && p[2] == 0xBF) {
			p += 3;
			len -= 3;
