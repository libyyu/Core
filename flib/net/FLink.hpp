...
public:
	...
	inline bool Connect(FSockAddr& addr);
	inline bool Close();
	inline bool Send(const void* buf,size_t nlen);
	inline bool Recv(size_t minSize = 0);
protected:
	inline bool FixBuffer(FBuffer* pbuffer);
	inline bool _Recv(FBuffer* pbuffer,size_t minSize = 0);
protected:
	size_t          _nTimeOut;
	spFBufferT     _pBuffer;
	bool            _isConnected;
...
inline spFBufferT FLink::GetBuffer()
{
    if(!_pBuffer)
			_pBuffer = spFBufferT(new FBuffer);
	return _pBuffer;
}
void FLink::SetTimeOut(size_t nTimeOut)
{
    if(0 == nTimeOut)
        nTimeOut = TimeOutSec;
    _nTimeOut = nTimeOut;
}
bool FLink::Connect(FSockAddr& addr)
{
...
bool FLink::Send(const void* buf,size_t nlen)
{
    if(_s.WaitWriteable(_nTimeOut))
        return _s.Send((char*)buf,nlen) == nlen;
    return false;
}
bool FLink::FixBuffer(FBuffer* pbuffer)
{
    assert(pbuffer);
...
bool FLink::Recv(size_t minSize /*= 0*/)
{
    spFBufferT theBuffer(GetBuffer());
    return _Recv(theBuffer.get(),minSize);
}
bool FLink::_Recv(FBuffer* pbuffer,size_t minSize/* = 0*/)
{
    assert(IsConnected());
...
inline bool FHttpLink::SendHttpRequest(const std::string& s)
{
    assert(!s.empty());
...
inline bool FHttpLink::ParseResponseHeader(bool &bGZip,bool &bChunked,int &nContentLen,int& nError)
{
...
inline bool FHttpLink::GetAllResponse()
{
    if(_pBuffer)
        _pBuffer->clear();
...
inline bool FHttpLink::RecvByContentLen(const int nContenLen)
{
    if(nContenLen > 0)
    {
