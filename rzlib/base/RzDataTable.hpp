...
一个简单的二维文本表格切分类
*/
#ifndef __RZDATATABLE_HPP__
#define __RZDATATABLE_HPP__
#include <sstream>
#include <vector>
#include <string>
#include <cassert>
...
public:
	CRzDataTable() {}
	CRzDataTable(CRzDataTable& ths)
	{
		Empty();
		_table.swap(ths._table);
	}
	CRzDataTable& operator = (CRzDataTable& ths)
	{
		Empty();
		_table.swap(ths._table);
		return *this;
	}
	CRzDataTable(TABLE& tb)
	{
		Empty();
		_table.swap(tb);
	}
	CRzDataTable& operator = (TABLE& tb)
	{
		Empty();
		_table.swap(tb);
		return *this;
	...
	void Transform(size_t startrow, size_t startcolumn, size_t row_count, size_t col_count, CRzDataTable & new_table)
	{
		new_table.Empty();
